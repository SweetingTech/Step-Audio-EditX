...
def load_model(args) -> StepAudioTTS:
    step_audio_editx_model_path = args.model_path + "/Step-Audio-EditX-VLLM"
    step_audio_tokenizer_path = args.model_path + "/Step-Audio-Tokenizer"
...
def infer():
    ...
    output_audio, output_sr  = model.edit(
        ...
        text="这是一条测试音频，[Laughter]尝试各种功能是否正常运行。",
    )
    ...
    ## speed iter_2
    output_audio, output_sr  = model.edit(
        input_audio_path="./speed_output.wav",
        audio_text=prompt_audio_text,
        edit_type="speed",
        edit_info="more faster",
    )
