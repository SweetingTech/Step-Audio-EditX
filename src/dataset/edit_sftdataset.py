...
class EditSFTDataset(TorchDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            lazy: If True, data is not loaded into memory during init; it iterates via a generator.
        """
    ...
    def process_item_raw(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        target_audio_str = self.processing_class.decode(processed_base['target_vq02vq06'])
        ...
        if processed_base['task_type'] == "edit":
            ...
            messages = [
                ...
                {"role": "assistant", "content": f"{target_audio_str}"}
            ]
        else:
    ...
    def __len__(self) -> int:
        if self.lazy:
             # In lazy mode, exact length is unknown unless metadata is pre-read.
             # Return 0 or raise error; Dataset.from_generator usually doesn't require this.
             return 0 
...
if __name__ == "__main__":
    ...
    if os.path.exists(tokenizer_path):
        ...
        print(f"Warning: {tokenizer_path} not found, using dummy.")
        processing_class = None # 测试时需处理 None

    if processing_class:
        try:
            print(f"\n--- 正在初始化 HF Dataset (Lazy) ---")
            
            # 3. 获取 HF Dataset
            hf_ds = create_edit_sftdataset(
                json_file=data_paths, 
                max_text_length=512,
                max_audio_tokens=2048,
                processing_class=processing_class
            )

            print(f"Dataset 类型: {type(hf_ds)}")
            
            
            print(f"数据集样本数: {len(hf_ds)}")
            
            if len(hf_ds) > 0:
                print("\n--- 正在测试 dataset[0] ---")
                sample = hf_ds[0] # 支持随机访问
